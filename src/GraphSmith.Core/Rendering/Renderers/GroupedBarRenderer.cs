...
{
    public class GroupedBarRenderer : IBarRenderer
    {
        public void Render(IRenderContext context, BarChartModel model)
        {
            ...
            double barStartX = baseX + (xStep - model.BarWidth * seriesCount) / 2;
            for (int c = 0; c < categoryCount; c++)
            {
                double innerX = barStartX + c * xStep;
            ...
            if (model.ShowXLabels)
            {
                for (int c = 0; c < categoryCount; c++)
                {
                    double labelX = barStartX + c * xStep + model.BarWidth * seriesCount / 2 - 10;
